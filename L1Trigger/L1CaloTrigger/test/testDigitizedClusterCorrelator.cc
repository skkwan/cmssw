...
bool passItemizedDigitizedConstructorTest(void) {
  ...
  l1tp2::DigitizedClusterCorrelator dc = l1tp2::DigitizedClusterCorrelator((ap_uint<12>)0x1,
                                                                           ...
                                                                           (ap_uint<1>)0x1,
                                                                           isDigitizedInputs);
  return ((dc.pt() == 0x1) && (dc.eta() == 0x1) && (dc.phi() == 0x1) && (dc.etaCr() == 0x1) && (dc.phiCr() == 0x1) &&
          (dc.hoe() == 0x1) && (dc.iso() == 0x1) && (dc.fb() == 0x1) && (dc.timing() == 0x1) && (dc.shape() == 0x1) &&
          (dc.brems() == 0x1) && (dc.cardNumber() == 0x1) && dc.passNullBitsCheck());
}
...
bool passMaxValuesConstructorTest(void) {
  ...
  l1tp2::DigitizedClusterCorrelator dc = l1tp2::DigitizedClusterCorrelator(
      maxPt, maxiEta, maxiPhi, maxiCrystal, maxiCrystal, maxHoE_dummy, true, 0, 0, true, 0, nGCTCard);
  return ((dc.pt() == 0xFFF) && (dc.eta() == maxiEta) && (dc.phi() == maxiPhi) && (dc.etaCr() == maxiCrystal) &&
          (dc.phiCr() == maxiCrystal) && (dc.hoe() == maxHoE_dummy) && (dc.iso() == true) && (dc.shape() == true) &&
          dc.passNullBitsCheck() && (dc.cardNumber() == nGCTCard));
}
