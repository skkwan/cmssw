...
int passItemizedDigitizedConstructorTest(void) {
  ...
  l1tp2::DigitizedTowerCorrelator tow =
      l1tp2::DigitizedTowerCorrelator(pt, hoe, fb, nGCTCard, nFiber, nTower, isFullyDigitizedInputs);
  return ((tow.et() == 0x1) && (tow.hoe() == 0x1) && (tow.fb() == 0x1) && (nGCTCard == 1) && (nFiber == 1) &&
          (nTower == 1));
}
